...
/*
    This checks that the request (GET) response codes are managed correctly and that
    the behaviour is the documented one.
    This also checks that wrong headers (too long headers or single header line)
    is rejected with a disconnection.
*/
...
#define URL5 "http://" MANGUSTA_TEST_BINDTO ":" MANGUSTA_TEST_BINDPORT R_URL5
...
static apr_status_t on_request_ready(mangusta_ctx_t * ctx, mangusta_request_t * req) {
    ...
    if (strstr(url, "test3") != NULL) {
        ...
        mangusta_response_status_set(req, 500, "Test must fail and we should not be here");
    }

    if (strstr(url, "test4") != NULL) {
        assert_string_equal(version, "HTTP/1.1");
        assert_string_equal(url, R_URL4);
        mangusta_response_status_set(req, 500, "Test must fail and we should not be here");
    }

    /* Stop the test when we have received a response on our last request */
    if (strstr(url, "test5") != NULL) {
        assert_string_equal(url, R_URL5);
        mangusta_context_stop(ctx);
    }
...
static void test_perform(void **UNUSED(foo)) {
    ...
    *(params + PARAMSIZE_BIG2 - 1) = '\0';
    ...
    curl_perform(ctx, URL5, CURL_HTTP_VERSION_1_1);
